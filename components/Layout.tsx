    ...
    <div className="min-h-screen flex flex-col">
      <header className="bg-[#F7F6F4]/80 backdrop-blur-md sticky top-0 z-50 px-8 py-4 flex items-center justify-between border-b border-black/5">
        <div className="flex items-center gap-10">
          ...
          >
            <div className="w-8 h-8 bg-[#121317] flex items-center justify-center text-white font-black tracking-tighter text-xl rounded-sm">A</div>
            <span className="text-xl font-bold tracking-tighter uppercase text-[#121317]">{APP_NAME}</span>
          </div>
          
          {isLoggedIn && (
            <nav className="hidden lg:flex items-center gap-8">
              <button onClick={() => setActiveTab('protect')} className={`nav-link ${activeTab === 'protect' ? 'opacity-100 font-semibold' : ''}`}>Sign Work</button>
              <button onClick={() => setActiveTab('verify')} className={`nav-link ${activeTab === 'verify' ? 'opacity-100 font-semibold' : ''}`}>Verify Forensic</button>
        ...
        <div className="flex items-center gap-8">
          {!isLoggedIn ? (
            <>
              <button 
                onClick={onLogin}
                className="text-sm font-medium text-[#2B2D33] hover:text-[#121317] transition-colors"
              >
                Log in
              </button>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <div className="text-right hidden sm:block">
                  <p className="text-[10px] font-bold text-[#6B6F76] uppercase tracking-widest group-hover:text-[#121317] transition-colors">Creator Account</p>
                  <p className="text-xs font-bold text-[#121317]">Jane Doe</p>
                </div>
                <div className="w-10 h-10 rounded-full border border-black/10 bg-white flex items-center justify-center text-xs font-bold shadow-sm group-hover:border-black/30 transition-all overflow-hidden">
                  JD
                </div>
              </div>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-4 w-56 bg-white hex-card border border-black/10 shadow-xl py-2 animate-in fade-in slide-in-from-top-2 duration-200 z-[60]">
                  <div className="px-4 py-3 border-b border-black/5 mb-1">
                    <p className="text-xs font-bold text-[#121317]">Jane Doe</p>
                    <p className="text-[10px] text-[#6B6F76] font-medium">jane.doe@authmark.ai</p>
                  </div>
                  
                  <button 
                    onClick={() => handleDashboardLink('signed')}
                    className="w-full text-left px-4 py-2 text-xs font-bold text-[#2B2D33] hover:bg-[#F7F6F4] hover:text-[#121317] transition-colors flex items-center gap-3"
                  >
                    <Icons.Shield />
                    Signed Assets
                  </button>
                  ...
                  <button 
                    ...
                    className="w-full text-left px-4 py-2 text-xs font-bold text-red-600 hover:bg-red-50 transition-colors flex items-center gap-3 mt-1"
                  >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-16">
            <div className="space-y-4">
              ...
              <ul className="space-y-3 text-sm font-medium text-[#2B2D33]">
                <li className="hover:text-black cursor-pointer transition-colors" onClick={() => setActiveTab('protect')}>Signature Engine</li>
                <li className="hover:text-black cursor-pointer transition-colors" onClick={() => setActiveTab('verify')}>Forensic Scan</li>
            ...
            <div className="space-y-4">
              ...
              <ul className="space-y-3 text-sm font-medium text-[#2B2D33]">
                ...
                <li className="hover:text-black cursor-pointer transition-colors">HMAC Protocol</li>
                <li className="hover:text-black cursor-pointer transition-colors">Auth</li>
              </ul>
            </div>
            <div className="space-y-4">
